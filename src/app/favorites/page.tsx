...
// The saved list is PAGINATED: besides scraping the current DOM, the bookmarklet
// fetches ?page=1..N with the user's session and extracts /plp/ links from each
// page's HTML (covers both server-rendered anchors and the embedded __NEXT_DATA__
// JSON, where slashes may arrive \-escaped), stopping at the first page with none.
const BOOKMARKLET = String.raw`javascript:(async function(){var u=new Set();function add(p){p=p.replace(/\\/g,'').split('?')[0];u.add(p.indexOf('http')===0?p:'https://www.propertyfinder.ae'+p)}function grab(t){t=t.replace(/\\\//g,'/');var m=t.match(/(https?:\/\/[^"'\s]*)?\/(?:[a-z]{2}\/)?plp\/[^"'\s?#\\]*-\d+\.html/g)||[];m.forEach(add);return m.length}document.querySelectorAll('a[href]').forEach(function(a){var h=(a.href||'').split('?')[0];if(/\/plp\/.*-\d+\.html$/.test(h))u.add(h)});var n=document.getElementById('__NEXT_DATA__');if(n)grab(n.textContent);var base=location.origin+location.pathname,pages=0;for(var p=1;p<=40;p++){try{var r=await fetch(base+'?page='+p,{credentials:'include'});if(!r.ok)break;var c=grab(await r.text());if(!c)break;pages=p}catch(e){break}}var l=[...u];if(!l.length){alert('No encontre pisos guardados aqui. Abre tu pagina de Guardados de Property Finder con la sesion iniciada.');return}var t=l.join('\n');if(navigator.clipboard){navigator.clipboard.writeText(t).then(function(){alert(l.length+' piso(s) copiados de '+(pages||1)+' pagina(s). Abre Brickwise y pulsa "Pegar de Property Finder".')},function(){prompt('Copia estos enlaces y pegalos en Brickwise:',t)})}else{prompt('Copia estos enlaces y pegalos en Brickwise:',t)}})();`;
