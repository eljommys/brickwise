...
// PF's saved list is a client-rendered SPA, so URL guessing alone can't sweep it.
// The bookmarklet detects how the page actually paginates and drives it: a
// href-less "next" button gets clicked (waiting for each render), real pagination
// links get fetched, and failing both it tries infinite-scroll plus a ?page=N
// fallback that stops when a page repeats. Listing links are harvested from the
// DOM and any HTML/JSON text (slashes may arrive \-escaped in __NEXT_DATA__).
const BOOKMARKLET = String.raw`javascript:(async function(){var u=new Set();function norm(p){p=p.replace(/\\/g,'').split('?')[0];return p.indexOf('http')===0?p:'https://www.propertyfinder.ae'+p}function grab(t){t=t.replace(/\\\//g,'/');var m=t.match(/(https?:\/\/[^"'\s]*)?\/(?:[a-z]{2}\/)?plp\/[^"'\s?#\\]*-\d+\.html/g)||[];m.forEach(function(x){u.add(norm(x))});return m.length}function harvest(){document.querySelectorAll('a[href]').forEach(function(a){var h=(a.href||'').split('?')[0];if(/\/plp\/.*-\d+\.html$/.test(h))u.add(h)});var n=document.getElementById('__NEXT_DATA__');if(n)grab(n.textContent)}function inNoise(e){for(var x=e;x&&x.className!==undefined;x=x.parentElement){if(/carousel|slider|swiper|gallery/i.test(String(x.className)))return true}return false}function usable(e){return !!e&&!e.disabled&&e.getAttribute('aria-disabled')!=='true'&&!/disabled/i.test(String(e.className))}function nextBtn(){var c=document.querySelector('[rel="next"]');if(usable(c))return c;var pool=[].slice.call(document.querySelectorAll('[class*="agination"] a,[class*="agination"] button,nav a,nav button,a,button'));for(var i=0;i<pool.length;i++){var e=pool[i];if(inNoise(e))continue;var al=e.getAttribute('aria-label')||'';var tx=(e.textContent||'').trim();if(/next|siguiente/i.test(al)||tx==='>'||tx==='›'||tx==='→'){if(usable(e))return e}}return null}function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}harvest();var initial=u.size;var b=nextBtn();if(b&&!b.getAttribute('href')){for(var it=0,stale=0;it<60&&stale<2;it++){b=nextBtn();if(!b||b.getAttribute('href'))break;var s0=u.size;b.click();await sleep(1400);harvest();if(u.size===s0)stale++;else stale=0}}else if(b&&b.getAttribute('href')){var seen=new Set(),q=[b.href];document.querySelectorAll('a[href*="page="]').forEach(function(a){q.push(a.href)});while(q.length&&seen.size<40){var url=q.shift();if(seen.has(url))continue;seen.add(url);try{var r=await fetch(url,{credentials:'include'});if(!r.ok)continue;var t0=await r.text();grab(t0);(t0.match(/href="[^"]*page=\d+[^"]*"/g)||[]).forEach(function(h){var v=h.slice(6,-1).replace(/&amp;/g,'&');q.push(v.indexOf('http')===0?v:location.origin+v)})}catch(e){}}}else{for(var it2=0,st2=0;it2<30&&st2<3;it2++){var s2=u.size;window.scrollTo(0,(document.body||{scrollHeight:9e9}).scrollHeight);await sleep(1200);harvest();if(u.size===s2)st2++;else st2=0}}if(u.size<=initial){var base=location.origin+location.pathname,last='';for(var p=1;p<=40;p++){try{var r2=await fetch(base+'?page='+p,{credentials:'include'});if(!r2.ok)break;var t2=await r2.text();var sig=(t2.match(/-\d+\.html/g)||[]).join(',');if(!sig||sig===last)break;last=sig;grab(t2)}catch(e){break}}}var l=[...u];if(!l.length){alert('No encontre pisos guardados aqui. Abre tu pagina de Guardados de Property Finder con la sesion iniciada.');return}var t=l.join('\n');function done(){alert(l.length+' piso(s) copiados. Abre Brickwise y pulsa "Pegar de Property Finder".')}if(navigator.clipboard){navigator.clipboard.writeText(t).then(done,function(){prompt('Copia estos enlaces y pegalos en Brickwise:',t)})}else{prompt('Copia estos enlaces y pegalos en Brickwise:',t)}})();`;
